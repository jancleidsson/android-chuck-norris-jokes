...
class GetFavoriteJokeTest : TestCase() {

    @Test
    fun testInvoke_callsGetFavoriteJoke() = runBlockingTest {
        val jokeRepository = mock(JokeRepository::class.java)
        val joke = mock(Joke::class.java)
        val getFavoriteJoke = GetFavoriteJoke(jokeRepository)

        getFavoriteJoke.invoke(joke)
        verify(jokeRepository, times(1)).getFavoriteJoke(joke)
    }
}